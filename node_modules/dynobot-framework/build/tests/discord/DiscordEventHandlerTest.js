...
describe("The class DiscordEventHandler", function () {
    ...
    describe("The method isInitEvent", function () {
        it("Returns false because the event is not an init event", function () {
            //Arrange
            let Event = new DiscordEventHandler_1.DiscordEventHandler("message", this._apiEvents);
            //Act
            let isInitEvent = Event.isInitEvent();
            //Assert
        ...
        it("Returns true because the event is an init event", function () {
            //Arrange
            let Event = new DiscordEventHandler_1.DiscordEventHandler("ready", this._apiEvents);
            //Act
            let isInitEvent = Event.isInitEvent();
            //Assert
    ...
    describe("The method getApiEventName", function () {
        it("Returns the original unwrapped api event name", function () {
            //Arrange
            let Event = new DiscordEventHandler_1.DiscordEventHandler("serverMemberAdd", this._apiEvents);
            //Act
            let apiEventName = Event.getApiEventName();
            //Assert
    ...
    describe("The method getWrappedObject", function () {
        it("Returns the wrapped object of the event", function () {
            ...
            let apiObject = {};
            //Act
            let WrappedObject = Event.getWrappedObject(apiObject);
            //Assert
            assert.strictEqual(WrappedObject instanceof DiscordMessage_1.DiscordMessage, true, "The api object was wrapped correctly.");
        });
        it("Returns the api object because it should not be wrapped", function () {
            //Arrange
            let Event = new DiscordEventHandler_1.DiscordEventHandler("error", this._apiEvents);
            let apiObject = new Error("test error");
            //Act
            let WrappedObject = Event.getWrappedObject(apiObject);
            //Assert
        ...
        it("Returns null if the event has no return value and shall not be wrapped", function () {
            ...
            let Event = new DiscordEventHandler_1.DiscordEventHandler("ready", this._apiEvents);
            //Act
            let WrappedObject = Event.getWrappedObject(undefined);
            //Assert
    ...
    describe("The method wrap", function () {
        it("Emits the wrapped event and hands over a wrapped object", function () {
            //Arrange
            let OriginalEmitter = new events_1.EventEmitter();
            let WrappedEmitter = new events_1.EventEmitter();
            ...
            let getWrappedObjectStub = sinon.stub(Event, "getWrappedObject").returns(new DiscordMessage_1.DiscordMessage(eventObject));
            //Act
            ...
            WrappedEmitter.on("message", eventObject => {
                ...
                assert.strictEqual(wrappedEmitStub.getCall(0).args[1] instanceof DiscordMessage_1.DiscordMessage, true, "The object was wrapped correctly.");
            });
        });
...
//# sourceMappingURL=DiscordEventHandlerTest.js.map